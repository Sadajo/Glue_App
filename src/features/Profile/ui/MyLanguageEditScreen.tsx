...
import {useUpdateMainLanguage} from '../api/hooks';
...
const MyLanguageEditScreen = () => {
  const navigation = useNavigation<any>();
  const {t} = useTranslation();
  const {myPageInfo, isLoading} = useMyPage();
  const updateMainLanguage = useUpdateMainLanguage();
  ...
  useEffect(() => {
    if (myPageInfo) {
      setSelectedLanguage(myPageInfo.mainLanguage);
      setSelectedLevel(myPageInfo.mainLanguageLevel);
    }
  }, [myPageInfo]);
  ...
  const handleUpdateLanguage = async () => {
    try {
      console.log('언어 수정 요청:', {
        language: selectedLanguage,
        languageLevel: selectedLevel,
      });

      await updateMainLanguage.mutateAsync({
        language: selectedLanguage,
        languageLevel: selectedLevel,
      });

      console.log('언어 수정 성공');
      Alert.alert('성공', '나의 언어가 수정되었습니다.');
      navigation.goBack();
    } catch (error) {
      console.error('언어 수정 에러:', error);
      Alert.alert(
        '오류',
  ...
  return (
    <SafeArea>
      <BackButtonHeader title="나의 언어" />
      <View style={styles.container}>
        <View style={styles.content}>
          {/* 언어 선택 */}
          <View style={styles.section}>
            <TypographyText style={styles.label}>나의 언어</TypographyText>
            <TouchableOpacity
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            ...
            disabled={updateMainLanguage.isPending}>
            <TypographyText style={styles.modifyButtonText}>
              {updateMainLanguage.isPending ? '수정 중...' : '수정하기'}
            </TypographyText>
          </TouchableOpacity>
        </View>
      </View>
